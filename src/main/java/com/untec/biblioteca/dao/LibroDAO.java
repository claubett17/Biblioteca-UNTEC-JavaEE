...
public class LibroDAO {
		...
		try (Connection conn = ConexionDB.getConexion();
			...
			while (rs.next()) {
				...
				Libro libro = new Libro(rs.getInt("idLibro"), rs.getString("isbn"), rs.getString("titulo"),
						rs.getString("autor"), rs.getInt("anioPublicacion"), rs.getString("editorial"),
						rs.getString("categoria"), rs.getBoolean("disponible"));
				libros.add(libro);
			}
		}
		return libros;
	...
	public Libro buscarPorId(int id) throws SQLException {
		...
		try (Connection conn = ConexionDB.getConexion(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					return new Libro(rs.getInt("idLibro"), rs.getString("isbn"), rs.getString("titulo"),
							rs.getString("autor"), rs.getInt("anioPublicacion"), rs.getString("editorial"),
							rs.getString("categoria"), rs.getBoolean("disponible"));
				}
			}
	...
	public void insertar(Libro libro) throws SQLException {
		...
		String sql = "INSERT INTO libros (isbn, titulo, autor, anioPublicacion, editorial, categoria, disponible) VALUES (?, ?, ?, ?, ?, ?, ?)";
	...
	public void actualizar(Libro libro) throws SQLException {
		...
		String sql = "UPDATE libros SET isbn=?, titulo=?, autor=?, anioPublicacion=?, editorial=?, categoria=?, disponible=? WHERE idLibro=?";
