    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            switch (accion) {
                ...
                case "solicitar":
                    int idLibroSol = Integer.parseInt(request.getParameter("id"));
                    Libro libroSol = libroDAO.buscarPorId(idLibroSol);

                    if (libroSol == null || !libroSol.isDisponible()) {
                        request.setAttribute("error", "El libro no está disponible");
                        request.setAttribute("prestamos", prestamoDAO.listarPorUsuario(usuario.getIdUsuario()));
                        request.getRequestDispatcher("/views/usuario/prestamos.jsp").forward(request, response);
                        return;
                    }
                    ...
                    Prestamo nuevoPrestamo = new Prestamo(0, usuario.getIdUsuario(), idLibroSol, hoy.toString(),
                            fechaLimite.toString(), null, "ACTIVO");
                    
                    prestamoDAO.insertar(nuevoPrestamo);

                    libroSol.setDisponible(false);
                    libroDAO.actualizar(libroSol);
